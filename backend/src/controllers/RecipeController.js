...
export const RecipeController = {
  async search (req, res, next)  {
    try {
      const { data, total } = await RecipeService.search(req.query)
  ...
  async insert (req, res, next) {
    try {
      ...
      const recipe = await RecipeService.insert(data)
  ...
  async update (req, res, next)  {
    try {
      ...
      const recipe = await RecipeService.update(id, updateData)
  ...
  async remove (req, res, next) {
    try {
      ...
      const deletedUser = await RecipeService.remove(Number(id));
      if (!deletedUser) {
        return res.status(404).json({
          errors: [{ message: 'Receita não encontrada.' }]
        })
      }
