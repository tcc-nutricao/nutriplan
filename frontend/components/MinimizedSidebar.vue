<template>
  <aside class="w-30 transform transition-transform duration-300 z-50">
    ...
    <div class="w-full h-full flex flex-col items-center p-5 
                ...
                drop-shadow-xl justify-between"
    >
        <div class="cursor-pointer bg-white flex flex-col items-center mb-3 p-2 rounded-xl transition drop-shadow-xl">
            <svg
        ...
        <i 
            class="fa-solid fa-right-from-bracket text-white text-lg hover:font-semibold cursor-pointer"
            @click="logout"
            title="Sair"
        />
    </div>
...
const items = [
  { icon: 'fa-map', iconLabel: 'Plano Alimentar', route: '/' },
  { icon: 'fa-book', iconLabel: 'Diário Alimentar', route: '/' },
  { icon: 'fa-spinner', iconLabel: 'Meu Progresso', route: '/' },
  { icon: 'fa-bowl-food', iconLabel: 'Receitas', route: '/' },
  { icon: 'fa-people-group', iconLabel: 'Meus Grupos', route: '/' }
]
