<template>
  <aside class="w-20 transform transition-transform duration-300 z-50 ml-2 mr-5">
    <div class="flex justify-end">
      <button @click="$emit('minimized', true)" class="absolute z-50 top-3 -right-3 cursor-pointer p-1 bg-white rounded-full w-6 h-6 flex items-center justify-center shadow transition hover:scale-[115%] active:scale-95">
    ...
    <div class="w-full flex flex-col items-center p-5 bg-gradient-to-br from-p-500 via-p-600 to-p-700 rounded-xl h-fit drop-shadow-xl">
        ...
        />
        <i class="fa-solid fa-right-from-bracket ml-1 text-white hover:text-danger-light text-lg font-semibold hover:scale-[115%] active:scale-95 transition cursor-pointer" @click="logout" title="Sair"/>
    </div>
  </aside>
</template>
...
const emit = defineEmits(['minimized', 'update:activeItem'])
...
const handleProfileClick = () => {
    ...
    if (listItemRef.value) {
        listItemRef.value.clearSelection();
    }
}
